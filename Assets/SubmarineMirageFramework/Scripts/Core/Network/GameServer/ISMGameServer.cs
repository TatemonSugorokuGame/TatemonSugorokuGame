...
namespace SubmarineMirage.Network {
	...
	public interface ISMGameServer : ISMStandardBase {
		...
		Subject<SMGameServerRoom> _currentRoomEvent { get; }
		/// <summary>部屋一覧のイベント</summary>
		Subject< List<SMGameServerRoom> > _roomsEvent { get; }
		/// <summary>失敗のイベント</summary>
		Subject<string> _errorEvent { get; }

		/// <summary>全て接続完了か？</summary>
		public bool _isConnect { get; }
		...
		UniTask<bool> ConnectOnline( string playerName );
		/// <summary>● オフライン接続</summary>
		UniTask<bool> ConnectOffline( string playerName );
		...
		UniTask<bool> EnterRoom( SMGameServerRoom room, string inputPassword );
	}
}